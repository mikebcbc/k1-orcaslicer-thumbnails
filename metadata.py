...
class BaseSlicer(object):
    ...
    def parse_thumbnails(self) -> Optional[List[Dict[str, Any]]]:
        ...
        if len(parsed_matches) > 0 and not has_miniature:
            ...
            thumb_path_creality = os.path.join(
                thumb_dir, f"{thumb_base}.gcode.png")
            # read file
            try:
        ...
        thumb_full_name = largest_match['relative_path'].split("/")[-1]
        thumb_path = os.path.join(thumb_dir, f"{thumb_full_name}")
        rel_path_creality = os.path.join(".thumbs", f"{thumb_base}.gcode.png")
        thumb_path_creality = os.path.join(
            thumb_dir, f"{thumb_base}.gcode.png")
        # read file
        try:
            with Image.open(thumb_path) as im:
                # Create Creality thumbnail
                im.thumbnail((300, 300))
                im.save(thumb_path_creality, format="PNG")
                parsed_matches.insert(0, {
                    'width': im.width, 'height': im.height,
                    'size': os.path.getsize(thumb_path_creality),
                    'relative_path': rel_path_creality
                })
        except Exception as e:
            log_to_stderr(str(e))
